...
function send(res, status, html, type='text/html'){ res.writeHead(status, {'Content-Type': type, 'Cache-Control': 'no-store'}); res.end(html); }
function page(title, content){ const analytics = ANALYTICS_URL ? `<script defer src="${ANALYTICS_URL}/beacon.js?platform=legacymart"></script>` : ''; return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${esc(title)} | LegacyMart</title><link rel="stylesheet" href="/styles.css">${analytics}</head><body><header><a class="brand" href="/"><span class="mark">LM</span><span><b>LegacyMart</b><small>Trade • Community • Lasting Impact</small></span></a><nav><a href="/shop">Shop</a><a href="/faith-personified">Faith Personified</a><a href="/become-a-vendor">Become a Vendor</a><a href="/how-it-works">How It Works</a><a href="/about">About</a></nav></header><main>${content}</main><footer><b>LegacyMart</b><p>Buy with purpose. Sell with purpose. Build legacy.</p><p><a href="/privacy">Privacy</a> · <a href="/refunds">Refunds</a> · <a href="/terms">Terms</a> · <a href="/vendor-terms">Vendor Terms</a> · <a href="/admin">Admin</a></p></footer></body></html>`; }
...
http.createServer(async (req,res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/checkout') return send(res,200,checkout());
  ...
  if (req.method === 'GET' && url.pathname === '/health') return send(res,200,JSON.stringify({ok:true,service:'LegacyMart',checkout_enabled:CHECKOUT_ENABLED,payfast_mode:PAYFAST_MODE}),'application/json');
